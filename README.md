...
**實戰已翻譯 21+ 本書**，包含 Taleb（《黑天鵝》《反脆弱》）、Mandelbrot（碎形理論）、Power Law VC 創投歷史、AI 政策、軍事戰略、科普讀物等深度作品。
...
**輸出**：`superforecasting_bilingual.epub`（在原 epub 同資料夾）
...
...
[done] superforecasting_bilingual.epub  (15m32s, 0 retries)
```
...
```
[已翻過] superforecasting.epub 之前已翻譯，發現 2 個 _bilingual.epub：
    /Users/me/Downloads/books/superforecasting_bilingual.epub  (4.2 MB, 2026-05-21 23:32)
    /Users/me/Documents/superforecasting_bilingual.epub  (4.2 MB, 2026-05-15 14:18)
...
```bash
python scripts/translate.py --book superforecasting.epub --model haiku --force
```
...
**為什麼預設 opus？** 21 本實戰下來，opus 翻譯文學味重的書（Taleb《反脆弱》、Mandelbrot《市場的（錯誤）行為》）品質遠勝 sonnet/haiku。雖然會撞 5h 額度上限，但智能 sleep + pickle resume 會自動接續，使用者只要放著等。
...
### Q5: 翻譯品質怎麼樣？
A: opus 翻譯文學作品的品質明顯好於 sonnet/haiku。Taleb / Mandelbrot 這類用詞精準、隱喻多的作者，opus 能傳達原意；haiku 翻文學會像 Google Translate。技術書 / 金融書 sonnet 就夠用。

### Q6: 它會翻譯人名 / 公司名嗎？
A: 預設第一次出現時翻成「中文（English）」格式（例：「巴菲特（Warren Buffett）」），之後出現只用中文。如果想完全控制，用 glossary YAML 自訂。
...
### Q10: 翻譯結果可以商用嗎？
A: 翻譯本身的 copyright 屬於原書作者。**只供個人閱讀，不要散布翻譯成果**。本工具的程式碼是 MIT 授權，但你用它翻譯的書本 copyright 不受工具授權影響。
...
- **[Charles](https://github.com/bockybocky)** — 原始 skill 設計、方法論萃取、Windows / Iris 整合、21 本書實戰驗證
